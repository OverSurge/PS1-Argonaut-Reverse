...
def parse_files(paths: Union[List[Path], Tuple[Path]], separated_wads: bool):
    ...
    for i in range(len(wads_names)):
        try:
            ...
            if args.export_audio:
                wad_audio_folder_path = Path(args.export_audio) / wads_names[i][:-4]
                if not wad_audio_folder_path.exists():
                    wad_audio_folder_path.mkdir()
                elif wad_audio_folder_path.is_file():
                    raise FileExistsError
                wad_file.export_audio(wad_audio_folder_path, wads_names[i][:-4])
            ...
            if args.export_models:
                wad_file.export_experimental_models(Path(args.export_models) / wads_names[i][:-4],
                                                    wads_names[i][:-4])
            if args.export_levels:
                wad_level_folder_path = Path(args.export_levels) / "No actors - No lighting" / wads_names[i][:-4]
                wad_file.export_level(wad_level_folder_path, wads_names[i][:-4])
...
if args.export_textures:
    tex_path = Path(args.export_textures)
    if tex_path.is_file():
        raise FileExistsError
