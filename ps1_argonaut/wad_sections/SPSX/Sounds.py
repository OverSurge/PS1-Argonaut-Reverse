...
from ps1_argonaut.wad_sections.SPSX.VAGSoundData import MONO, STEREO, VAGSoundData
...
class Sound(BaseDataClass):
    def __init__(
        self,
        sampling_rate: int,
        flags: SoundEffectsAmbientFlags | DialoguesBGMsSoundFlags,
...
class _AmbientOrEffectSound(Sound):
    ...
    def parse(cls, data_in: BufferedIOBase, conf: Configuration, *args, **kwargs):
        ...
        data_in.seek(
            2, SEEK_CUR
        )  # "Compressed" sampling rate, see SPSX's documentation
...
class DialogueBGMSound(Sound):
    ...
    def parse(cls, data_in: BufferedIOBase, conf: Configuration, *args, **kwargs):
        ...
        sampling_rate = int(
            round(((int.from_bytes(data_in.read(2), "little") * 44100) / 4096))
        )
    ...
    def parse_vag(self, data_in: BufferedIOBase, conf: Configuration):
        self.vag = VAGSoundData.parse(
            ...
            sampling_rate=self.sampling_rate,
            n_channels=STEREO
            if DialoguesBGMsSoundFlags.IS_STEREO in self.flags
            else MONO,
        )
        super().parse_vag(data_in, conf)
